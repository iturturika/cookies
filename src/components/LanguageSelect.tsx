...
export default function LanguageSelect({data}:{data:any}){
  ...
  return(
    <div className="relative xl:w-40 h-11 flex items-center justify-between px-3 py-4 rounded-lg cursor-pointer lg:w-11 md:w-11 sm:w-11 w-11"
    ...
    >
      ...
      <p className="text-sm xl:flex lg:hidden md:hidden sm:hidden hidden" style={{color: 'rgb(150, 160, 175)'}}>{selected.language}</p>
      <img src={arrow} alt="arrow" className={onClick ? "rotate-0 xl:flex lg:hidden md:hidden sm:hidden hidden" : "rotate-180 xl:flex lg:hidden md:hidden sm:hidden hidden"}/>
      <ul 
          ...
          data.map((item:any) => {
            return (
              <li className="flex w-full h-9 hover:bg-blue-700 px-1 py-2 items-center rounded-md gap-x-5" key={item.value} onClick={() => {setSelected(item); setOnClick(false)}}>
                <img src={item.img} alt="russian" className="xl:w-4 rounded-sm lg:w-full md:w-full sm:w-full w-full"/>
                <p className="text-sm xl:flex lg:hidden md:hidden sm:hidden hidden" style={{color: 'rgb(150, 160, 175)'}}>{item.language}</p>
                {selected.value === item.value ? <img src={checkbox} alt="checkbox" className='xl:flex lg:hidden md:hidden sm:hidden hidden'/> : null}
              </li>
            )
          })
        }
